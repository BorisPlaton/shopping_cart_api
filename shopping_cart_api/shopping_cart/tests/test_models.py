...
class TestShoppingCart:
    ...
    def test_last_updated_at_returns_the_latest_date(self, create_product):
        cart = ShoppingCart.objects.create()
        baker.make(OrderedProduct, updated_at=datetime.now() - timedelta(30), cart=cart, product=create_product())
        latest_product = baker.make(OrderedProduct, cart=cart, product=create_product())
